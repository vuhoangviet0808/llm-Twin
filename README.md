...
## LLM-Assisted Reward Design Experiment
...
```shell
python main_train.py --drl td3 --reward llm_balanced --ep-num 300 --seed 0 --trained-uav --uav-benchmark-reward see --project-name llm_reward/llm_balanced_seed_0
```

Evaluation command:

```shell
python evaluate_reward.py --model-path data/storage/llm_reward/llm_balanced_seed_0 --drl td3 --reward llm_balanced --episodes 5 --seed 100
```

Kaggle batch command:

```shell
bash kaggle_run.sh
```

Outputs: training runs under `data/storage/llm_reward/`; evaluation files are under each model's `evaluation/`; summaries are under `data/storage/summary/`.
