...
parser.add_argument('--reward', type = str, required = True, default='see', help="which reward would you like to implement ['ssr', 'see', 'llm_balanced', 'llm_gated', 'llm_ratio']")
...
parser.add_argument('--seed', type = int, required = False, default=None, help='global reproducibility seed')
...
parser.add_argument('--uav-benchmark-reward', type=str, default='see', choices=['ssr', 'see'], help='reward name used by the pretrained UAV benchmark')
parser.add_argument('--project-name', type=str, default=None, help='optional output project name under data/storage')
...
UAV_BENCHMARK_REWARD = args.uav_benchmark_reward
PROJECT_NAME = args.project_name

# process the argument
assert DRL_ALGO in ['ddpg', 'td3'], "drl must be ['ddpg', 'td3']"
assert REWARD_DESIGN in ['ssr', 'see', 'llm_balanced', 'llm_gated', 'llm_ratio'], "invalid reward design"
...
# Apply the explicit global seed after imports (env.py has a legacy import-time
# NumPy seed) and before constructing the environment or agents.
...
if TRAINED_UAV:
    benchmark = f'data/storage/benchmark/{DRL_ALGO}_{UAV_BENCHMARK_REWARD}_benchmark'
    if DRL_ALGO == 'td3':
        agent_2.load_models(
...
print("Torch device:  "+str(torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')))
print("Reward design: "+str(REWARD_DESIGN))
print("Global seed:   "+str(GLOBAL_SEED))
meta_dic['reward_design'] = REWARD_DESIGN
meta_dic['global_seed'] = GLOBAL_SEED
meta_dic['uav_benchmark_reward'] = UAV_BENCHMARK_REWARD if TRAINED_UAV else None
