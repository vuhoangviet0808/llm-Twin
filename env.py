...
from reward_design import extract_reward_components, compute_llm_reward
...
#class MiniSystem(K=1):
    ...
    def __init__(self, UAV_num = 1, RIS_num = 1, user_num = 1, attacker_num = 1, fre = 28e9, \
        ...
        store_list = ['beamforming_matrix', 'reflecting_coefficient', 'UAV_state', 'user_capacity', 'secure_capacity', 'attaker_capacity','G_power', 'reward', 'reward_raw', 'positive_ssr', 'secrecy_violation', 'power_violation', 'energy_raw', 'energy_norm', 'UAV_movement'])
        ...
        valid_rewards = ['ssr', 'see', 'llm_balanced', 'llm_gated', 'llm_ratio']
        if self.reward_design not in valid_rewards:
            raise ValueError("reward_design must be one of {}".format(valid_rewards))
    ...
    def step(self, action_0 = 0, action_1 = 0, G = 0, Phi = 0, set_pos_x = 0, set_pos_y = 0):
        ...
        # 7 get reward. The original ssr/see branch is intentionally retained.
        reward = self.reward()
        components = extract_reward_components(
            self, v_t, get_energy_consumption, ENERGY_MIN, ENERGY_MAX)
        reward_raw = reward
        ...
        if self.reward_design == 'see':
            ...
            reward_raw = reward
        elif self.reward_design in ('llm_balanced', 'llm_gated', 'llm_ratio'):
            reward_raw, reward = compute_llm_reward(self.reward_design, components)
        ######################################################
        
        # 8 calculate if UAV is cross the bourder
        if self.reward_design in ('ssr', 'see'):
            reward = math.tanh(reward) # preserve the original single tanh
        done = False
        ...
        self.data_manager.store_data([reward_raw], 'reward_raw')
        for key in ('positive_ssr', 'secrecy_violation', 'power_violation', 'energy_raw', 'energy_norm'):
            self.data_manager.store_data([components[key]], key)
        info = {
            'reward_design': self.reward_design,
            'reward_raw': reward_raw,
            'positive_ssr': components['positive_ssr'],
            'secrecy_violation': components['secrecy_violation'],
            'power_violation': components['power_violation'],
            'energy_raw': components['energy_raw'],
            'energy_norm': components['energy_norm'],
            'UAV_movement': [move_x, move_y],
        }
